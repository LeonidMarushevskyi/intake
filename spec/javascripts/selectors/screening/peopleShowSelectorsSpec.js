...
describe('peopleCardSelectors', () => {
  ...
  describe('getFormattedPersonInformationSelector', () => {
    ...
    it('includes the formatted languages for people', () => {
    ...
    it('includes the formatted date of birth for people', () => {
      const participants = [{id: '1', date_of_birth: '2014-01-15'}]
      const state = fromJS({participants})
      expect(getFormattedPersonInformationSelector(state, '1').get('date_of_birth')).toEqual('01/15/2014')
    })
    it('includes the approximate age for people', () => {
      const participants = [{id: '1', approximate_age: '9', approximate_age_units: 'dog years'}]
      const state = fromJS({participants})
      expect(getFormattedPersonInformationSelector(state, '1').get('approximate_age')).toEqual('9 dog years')
    })
    it('includes the formatted social security number for people', () => {
      const participants = [{id: '1', ssn: '123456789'}]
      const state = fromJS({participants})
      expect(getFormattedPersonInformationSelector(state, '1').get('ssn')).toEqual('123-45-6789')
    })
    it('includes the formatted races for people', () => {
      const participants = [
        {id: '1', races: [
    ...
    it('includes the formatted ethnicity for people', () => {
      const participants = [
