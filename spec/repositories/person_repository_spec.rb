...
describe PersonRepository do
  describe '.create' do
    ...
    before do
      expect(API).to receive(:make_api_call)
        .with('/api/v1/people', :post, 'first_name' => 'New Person')
        .and_return(response)
    end

    it 'returns the created person' do
      created_person = described_class.create(person)
  ...
  describe '.find' do
    ...
    before do
      expect(API).to receive(:make_api_call)
        .with("/api/v1/people/#{person_id}", :get)
        .and_return(response)
    end

    it 'returns the existing person' do
      existing_person = described_class.find(person_id)
  ...
  describe '.update' do
    ...
    context 'when person has an id' do
      ...
      before do
        expect(API).to receive(:make_api_call)
          .with("/api/v1/people/#{person_id}", :put, person.as_json(except: :id))
          .and_return(response)
      end

      it 'returns the updated person' do
        updated_person = described_class.update(person)
