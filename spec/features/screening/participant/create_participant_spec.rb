...
feature 'Edit Screening' do
  ...
  context 'adding a sensitive participant from search results' do
    ...
    scenario 'with NO privileges to add sensitive' do
      Feature.run_with_activated(:authentication) do
        visit edit_screening_path(id: existing_screening.id, token: insensitive_token)
        sensitive_homer_attributes = build_participant_from_person_and_screening(
          homer,
          existing_screening
        )
        sensitive_participant_homer = FactoryGirl.build(:participant, sensitive_homer_attributes)
        created_participant_homer = FactoryGirl.create(
          :participant,
          sensitive_participant_homer.as_json
        )
        stub_request(:post, host_url(ExternalRoutes.intake_api_participants_path))
        ...
        expect(a_request(:post, host_url(ExternalRoutes.intake_api_participants_path))
          .with(json_body(sensitive_participant_homer.to_json(except: :id)))).to have_been_made
        ...
        expect(page)
          .to_not have_selector(edit_participant_card_selector(created_participant_homer.id))
      end
    end

    scenario 'with privileges to add sensitive' do
      Feature.run_with_activated(:authentication) do
        ...
        expect(page).to have_selector(edit_participant_card_selector(created_participant_homer.id))
      end
    end
  end
